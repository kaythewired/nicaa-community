...
import townUnionDocumentsJson from "./town-union-documents.json";
...
const townUnionNames = Object.keys(townUnionDocumentsJson).sort((first, second) =>
...
export const townUnions: TownUnion[] = townUnionNames.map((name) => ({
  ...
  roster: townUnionRostersJson[name as keyof typeof townUnionRostersJson] ?? "",
  documentUrl:
    townUnionDocumentsJson[name as keyof typeof townUnionDocumentsJson].url,
  documentName:
    townUnionDocumentsJson[name as keyof typeof townUnionDocumentsJson].name,
}));
